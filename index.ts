...
async function signPdf(pdfBuffer: Buffer, certificatePath: string, passphrase: string, targetPath: string) {
  const certPath = path.join(__dirname, certificatePath);
  console.log('signPdf --> certPath', certPath);
  ...
  console.log('signPdf --> targetPath', targetPath);
...
async function work() {
  const pdfPath = path.join(__dirname, 'key/bmgd/ndomino_fullstack.14.pdf');
  console.log('work --> pdfPath', pdfPath);
...
console.log('work --> start', __dirname);
